...
class HybridPlayerTracker:
    def __init__(self, video_path):
        ...
        print("=" * 70)
        print("HYBRID PLAYER TRACKER - MOG2 + HSV Color Tracking")
        print("=" * 70)
        print("Strategy:")
        print("  Phase 1: Use MOG2 to detect players")
        print("  Phase 2: Learn colors from detected players")
        print("  Phase 3: Switch to HSV color tracking")
        print("=" * 70)
        ...
        cv2.namedWindow('Hybrid Player Tracker', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('Hybrid Player Tracker', 1600, 900)
    ...
    def learn_color_from_detection(self, frame, detection, player_name):
        """Learn HSV color from MOG2 detection"""
        ...
        # Use the mask to get only player pixels (not background)
        ...
        h_values = player_pixels[:, 0]
        s_values = player_pixels[:, 1]
        v_values = player_pixels[:, 2]
        ...
        # Create HSV range (median ± 2.5*std)
        ...
        print(f"  Sampled {len(player_pixels)} pixels from detection")
        
        return hsv_filter
    
    def detect_by_hsv(self, frame, hsv_filter, search_region=None, zone_polygon=None, min_area=2000):
    ...
    def show_debug(self, p1_mask, p2_mask):
        ...
        if self.p1_hsv_filter:
            print(f"P1 HSV: {self.p1_hsv_filter}")
        if self.p2_hsv_filter:
            print(f"P2 HSV: {self.p2_hsv_filter}")
    ...
    def run(self):
        ...
        while True:
            ...
            cv2.imshow('Hybrid Player Tracker', display_resized)
...
if __name__ == "__main__":
    ...
    tracker = HybridPlayerTracker(video_path)
    tracker.run()

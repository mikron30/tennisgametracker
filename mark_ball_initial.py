"""
Interactive tool to mark initial position of the tennis ball.
Ball = Green marker
"""

import cv2
import numpy as np
from typing import Tuple, Optional


class BallMarker:
    def __init__(self, video_path: str):
        self.video_path = video_path
        self.window_name = "Mark Ball - Click on the ball, 'S' to save, 'Q' to quit"
        
        # Ball position (will be scaled to full video frame)
        self.ball_pos = None  # Green
        
        # Display frame
        self.display_frame = None
        self.original_frame = None
        self.display_scale = 1.0
        
        # Screen dimensions
        self.screen_width = 1920
        self.screen_height = 1080
    
    def _resize_frame_to_fit_screen(self, frame: np.ndarray) -> Tuple[np.ndarray, float]:
        """Resize frame to fit screen while maintaining aspect ratio."""
        height, width = frame.shape[:2]
        
        scale_w = self.screen_width / width
        scale_h = self.screen_height / height
        scale = min(scale_w, scale_h, 1.0)
        
        new_width = int(width * scale)
        new_height = int(height * scale)
        
        resized = cv2.resize(frame, (new_width, new_height), interpolation=cv2.INTER_AREA)
        return resized, scale
    
    def _draw_marker(self, frame: np.ndarray, scale: float = 1.0) -> np.ndarray:
        """Draw ball marker on the frame."""
        result = frame.copy()
        
        # Draw Ball (Green) marker
        if self.ball_pos is not None:
            x, y = self.ball_pos
            display_x = int(x * scale)
            display_y = int(y * scale)
            
            # Green circle
            cv2.circle(result, (display_x, display_y), 20, (0, 255, 0), 2)
            # Green X
            size = 15
            cv2.line(result, (display_x - size, display_y - size), 
                    (display_x + size, display_y + size), (0, 255, 0), 2)
            cv2.line(result, (display_x - size, display_y + size), 
                    (display_x + size, display_y - size), (0, 255, 0), 2)
            cv2.putText(result, "Ball", (display_x - 30, display_y - 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        
        # Draw instructions
        instructions = [
            "Click on the ball to mark its position",
            "Press 'S' to save position",
            "Press 'Q' to quit"
        ]
        y_offset = 30
        for instruction in instructions:
            cv2.putText(result, instruction, (10, y_offset),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            y_offset += 30
        
        return result
    
    def _mouse_callback(self, event, x, y, flags, param):
        """Handle mouse events for marking ball."""
        scale = param
        
        if event == cv2.EVENT_LBUTTONDOWN:
            # Mark ball - convert from display scale to original
            self.ball_pos = (int(x / scale), int(y / scale))
            print(f"Ball marked at: ({self.ball_pos[0]}, {self.ball_pos[1]})")
            # Redraw
            self.display_frame = self._draw_marker(self.original_frame, scale)
    
    def _save_position(self):
        """Save ball position to file."""
        if self.ball_pos is None:
            print("\nError: Ball position must be marked before saving!")
            return False
        
        with open('ball_position.txt', 'w', encoding='utf-8') as f:
            f.write("Initial Ball Position\n")
            f.write("=" * 50 + "\n\n")
            f.write(f"Ball (Green): ({self.ball_pos[0]}, {self.ball_pos[1]})\n")
        
        print("\nBall position saved to: ball_position.txt")
        print(f"Ball (Green): ({self.ball_pos[0]}, {self.ball_pos[1]})")
        return True
    
    def run(self):
        """Run the interactive ball marking tool."""
        # Open video and get first frame
        cap = cv2.VideoCapture(self.video_path)
        if not cap.isOpened():
            print(f"Error: Could not open video file: {self.video_path}")
            return
        
        ret, frame = cap.read()
        cap.release()
        
        if not ret:
            print("Error: Could not read first frame from video")
            return
        
        video_height, video_width = frame.shape[:2]
        print(f"Video dimensions: {video_width}x{video_height}")
        
        # Resize frame to fit screen
        self.original_frame, self.display_scale = self._resize_frame_to_fit_screen(frame)
        
        print(f"Display scale: {self.display_scale:.3f}")
        print(f"Display size: {self.original_frame.shape[1]}x{self.original_frame.shape[0]}")
        
        # Initial draw
        self.display_frame = self._draw_marker(self.original_frame, self.display_scale)
        
        # Create window and set mouse callback
        cv2.namedWindow(self.window_name, cv2.WINDOW_NORMAL)
        cv2.setMouseCallback(self.window_name, self._mouse_callback, self.display_scale)
        
        print("\nMark Initial Ball Position")
        print("=" * 50)
        print("Instructions:")
        print("- Click on the ball to mark its position")
        print("- The ball should be above the server's head")
        print("- Press 'S' to save position")
        print("- Press 'Q' to quit")
        
        # Main loop
        saved = False
        while True:
            cv2.imshow(self.window_name, self.display_frame)
            key = cv2.waitKey(1) & 0xFF
            
            if key == ord('q') or key == ord('Q'):
                if not saved and self.ball_pos is not None:
                    print("\nWarning: Position not saved!")
                print("\nQuitting...")
                break
            elif key == ord('s') or key == ord('S'):
                if self._save_position():
                    saved = True
                    print("\nPosition saved! You can now quit or continue adjusting.")
        
        cv2.destroyAllWindows()


def main():
    video_path = "20251011124747503_FV3553362380_FV3553362.mp4"
    
    marker = BallMarker(video_path)
    marker.run()


if __name__ == "__main__":
    main()












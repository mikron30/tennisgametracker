...
hotspot_old = """                if selected_predicted_path_override and (static_hotspot or frame0_background):
                    debug = selected_meta_for_guard.get('predicted_path_hotspot_debug') or {}
                    print(
"""
hotspot_new = """                if selected_predicted_path_override and (static_hotspot or frame0_background):
                    debug = selected_meta_for_guard.get('predicted_path_hotspot_debug') or {}
                    ...
                    print(
"""
if hotspot_old not in text:
    raise SystemExit("hotspot insertion point not found")
text = text.replace(hotspot_old, hotspot_new, 1)

trigger_old = """        if zone is not None or contact_near:
        ...
        recent_speeds = [
"""
trigger_new = """        if zone is not None or contact_near:
            self._contact_local_ai_watch_until_frame = max(watch_until, current + 8)
            watch_active = True
...
if trigger_old not in text:
    raise SystemExit("trajectory trigger insertion point not found")
text = text.replace(trigger_old, trigger_new, 1)

handoff_old = """        # Keep AI for several frames after contact.  This prevents an immediate
        # handback to HSV while its search anchor is still the racket/player.
        if age >= 5 and int(state.get('outside_count', 0)) >= 3:
"""
handoff_new = """        # Keep AI for several frames after contact.  This prevents an immediate
        # handback to HSV while its search anchor is still the racket/player.
...
if handoff_old not in text:
    raise SystemExit("Local AI handoff insertion point not found")
text = text.replace(handoff_old, handoff_new, 1)

path.write_text(text, encoding="utf-8")
print("Applied automatic Local-AI trajectory hotspot trigger")

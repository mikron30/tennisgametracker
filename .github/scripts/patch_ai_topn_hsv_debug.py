...
start_marker = '    def _debug_local_ai_shadow_frame('
end_marker = '    def _try_local_ai_recovery('
start = text.find(start_marker)
end = text.find(end_marker, start)
if start < 0 or end < 0:
    raise RuntimeError('shadow debug method block not found')

method = '''    def _debug_local_ai_shadow_frame(self, frame, previous_position, normal_position):
...
if '--debug-local-ai-top-n' not in text:
    cli_marker = '    parser.add_argument("--disable-player-tracking", action="store_true",'
    if cli_marker not in text:
        raise RuntimeError('CLI insertion marker not found')
    cli = (
    ...
    text = text.replace(cli_marker, cli + cli_marker, 1)

pause_line = '                analyzer.pause_at_frame = args.pause_at_frame if sequence_index == 0 else None'
if '_debug_local_ai_top_n = args.debug_local_ai_top_n' not in text:
    if pause_line not in text:
        raise RuntimeError('analyzer runtime assignment marker not found')
    extra = (
        pause_line + '\n'
        '                analyzer._debug_local_ai_top_n = args.debug_local_ai_top_n\n'
        '                analyzer._debug_hsv_sweep = args.debug_hsv_sweep'
    )
    text = text.replace(pause_line, extra, 1)

path.write_text(text, encoding='utf-8')
print('patched Top-N and HSV sweep debug')

...
class FixedTunableTracker:
    def __init__(self, video_path):
        ...
        self.max_movement_per_frame = 150
        
        # Player positions
        self.p1_position = None
        self.p2_position = None
        
        # Frame where both were found
        self.learning_frame = None
        
        print("=" * 70)
        print("FIXED TUNABLE HSV TRACKER")
        print("=" * 70)
        print("Will use MOG2 until both players found")
        print("Then pause for HSV tuning")
        print("=" * 70)
        ...
        print("  'T' = Start HSV tracking (when in TUNING mode)")
        print("  'Q' = Quit")
        print("=" * 70)
    ...
    def apply_mog2(self, frame, learn_only=False):
        ...
        # Use frozen learning when detected
    ...
    def learn_shirt_colors(self, frame):
        ...
        print("=" * 70)
        print("HSV filters learned! Setting up tuning windows...")
        print("=" * 70)
    ...
    def setup_tuning_windows(self):
        ...
        print("\nTuning windows created!")
        print("Adjust trackbars and press 'T' to start HSV tracking")
    ...
    def draw_results(self, frame, p1_det, p2_det, p1_mask=None, p2_mask=None):
        ...
        if p1_det is not None:
            ...
            # Movement circle in tracking mode
            if self.mode == "HSV_TRACKING" and self.p1_position:
        ...
        if p2_det is not None:
            ...
            # Movement circle in tracking mode
            if self.mode == "HSV_TRACKING" and self.p2_position:
                cv2.circle(display, (cx, cy), self.max_movement_per_frame, (0, 255, 255), 1)
        ...
        if self.mode == "TUNING":
            cv2.putText(display, "Press 'T' to start tracking", (10, 80),
        ...
        if self.mode == "TUNING" and p1_mask is not None and p2_mask is not None:
            ...
            cv2.putText(p1_vis, f"P1 HSV DETECTION", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            cv2.putText(p1_vis, f"H:{self.p1_hsv['h_min']}-{self.p1_hsv['h_max']}", (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.putText(p1_vis, f"S:{self.p1_hsv['s_min']}-{self.p1_hsv['s_max']}", (10, 85),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.putText(p1_vis, f"V:{self.p1_hsv['v_min']}-{self.p1_hsv['v_max']}", (10, 110),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            ...
            cv2.putText(p2_vis, f"P2 HSV DETECTION", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            cv2.putText(p2_vis, f"H:{self.p2_hsv['h_min']}-{self.p2_hsv['h_max']}", (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.putText(p2_vis, f"S:{self.p2_hsv['s_min']}-{self.p2_hsv['s_max']}", (10, 85),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.putText(p2_vis, f"V:{self.p2_hsv['v_min']}-{self.p2_hsv['v_max']}", (10, 110),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.mode == "MOG2":
                ...
                if self.p1_position and self.p2_position and self.learning_frame is None:
                    self.learning_frame = self.current_frame
                    print(f"\n[FOUND] Both players at frame {self.current_frame}!")
                    self.learn_shirt_colors(frame)
                    self.setup_tuning_windows()
                    self.mode = "TUNING"
                    self.paused = True
                    print("\n[PAUSED] Adjust HSV trackbars, then press 'T' to start tracking")
            
            elif self.mode == "TUNING":
                # Show HSV detection results in real-time
                p1_det, p1_mask = self.detect_by_hsv(
                    frame, self.p1_hsv, self.p1_zone,
                    self.p1_min_area, self.p1_max_area, None)
                
                p2_det, p2_mask = self.detect_by_hsv(
                    frame, self.p2_hsv, self.p2_zone,
                    self.p2_min_area, self.p2_max_area, None)
            
            elif self.mode == "HSV_TRACKING":
                # Track with HSV and movement constraints
                p1_det, p1_mask = self.detect_by_hsv(
                    frame, self.p1_hsv, self.p1_zone,
                ...
                if p1_det is not None:
                    self.p1_position = p1_det
                    print(f"Frame {self.current_frame}: P1 at ({p1_det['center'][0]}, {p1_det['center'][1]})")
                else:
                    print(f"Frame {self.current_frame}: P1 NOT FOUND")
                
                if p2_det is not None:
                    self.p2_position = p2_det
                    print(f"Frame {self.current_frame}: P2 at ({p2_det['center'][0]}, {p2_det['center'][1]})")
                else:
                    print(f"Frame {self.current_frame}: P2 NOT FOUND")
            ...
            if key == ord('d'):
                if self.mode == "MOG2" or self.mode == "HSV_TRACKING":
            ...
            if not self.paused and (self.mode == "MOG2" or self.mode == "HSV_TRACKING"):
                self.current_frame = min(self.current_frame + 1, self.total_frames - 1)
...
if __name__ == "__main__":
    ...
    tracker = FixedTunableTracker(video_path)
    tracker.run()
